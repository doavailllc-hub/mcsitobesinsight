...
export default function Collections({ frontdesk = false, defaultFilter = 'active', showStats = true, defaultCompanyId = null }) {
  ...
  const filtered = useMemo(() => rows.filter(row => {
    const matchesStatus = filter === 'all' || (filter === 'due' ? row.status === 'active' && row.next_interest_date <= today() : row.status === filter);
  ...
  return <div className={frontdesk ? 'collection-frontdesk-content' : ''}>
    ...
    <div className="finance-page-toolbar collection-toolbar">
      ...
      <div className="finance-page-actions">
        <select className="finance-status-filter" value={filter} onChange={e => setFilter(e.target.value)}><option value="active">Active</option><option value="due">Due now</option><option value="closed">Closed</option><option value="all">All</option></select>
        <button className="primary-btn" onClick={() => { setError(''); setModal({ type: 'customer' }); }}><Plus size={16}/>Add customer</button>
      </div>
    </div>
    ...
    <section className="finance-list-card"><div className="finance-table-scroll"><table className="finance-table collection-table"><thead><tr><th>Customer</th><th>ID card</th><th>Principal</th><th>Monthly interest</th><th>Interest date</th><th>Status</th><th>Collected</th><th></th></tr></thead>
      <tbody>{filtered.map(row => { const due = row.status === 'active' && row.next_interest_date <= today(); return <tr key={row.id}><td><button className="collection-customer-link" onClick={() => openProfile(row)}><strong>{row.customer_name}</strong><small>{row.phone || row.company_name}</small></button></td><td>{row.id_card_number}{row.id_card_url && <a href={row.id_card_url} target="_blank" rel="noreferrer"><Eye size={13}/> View</a>}</td><td>{money(row.principal_amount,row.currency)}</td><td className="finance-money">{money(row.monthly_interest_amount,row.currency)}<small>{row.interest_type === 'percentage' ? `${row.interest_rate}%` : 'Flat'}</small></td><td><strong className={due ? 'collection-due-text' : ''}>{row.next_interest_date}</strong>{due && <small>{Number(row.days_overdue) > 0 ? `${row.days_overdue} days overdue` : 'Due today'}</small>}</td><td><span className={`finance-badge ${row.status === 'closed' ? 'approved' : due ? 'rejected' : 'pending'}`}>{row.status === 'closed' ? 'Closed' : due ? 'Payment due' : 'Active'}</span></td><td>{money(row.total_interest_collected,row.currency)}</td><td><div className="finance-actions"><button className="secondary-btn" onClick={() => openProfile(row)}>Profile</button>{row.status === 'active' && <button className="primary-btn collection-pay-btn" onClick={() => openPayment(row)}>Collect</button>}<button className="secondary-btn" onClick={() => toggleStatus(row)}>{row.status === 'active' ? 'Close' : 'Reopen'}</button></div></td></tr>; })}{!filtered.length && <tr><td colSpan="8" className="collection-empty">No collection customers found.</td></tr>}</tbody>
    </table></div></section>

    {modal?.type === 'customer' && <Modal title="Add collection customer" subtitle="Create the customer, verify identity and define the monthly interest agreement." close={() => setModal(null)} wide><form className="collection-customer-form" onSubmit={createCustomer}>
      ...
      <Footer saving={saving} cancel={() => setModal(null)} text="Add customer"/>
    </form></Modal>}
